      ...
      <Sidebar>
        <Flex direction="column" align="flex-start" justify="flex-start">
          ...
          >
            ...
            <Select bg="barber.900" mb={3} size="lg" w="85%" color='gray.200' onChange={(e) => handleChangeSelect(e.target.value)}>
              {haircuts?.map(item => (
                <option style={{  backgroundColor: 'gray.700', color: '#000' }} key={item?.id} value={item?.id}>{item.name}</option>
              ))}
            </Select>
